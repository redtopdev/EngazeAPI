/***************************************
# Object:  StoredProcedure [dbo].[ws_spGetAllEventsForUserInternal] to Fetch Detailed Event Info for user. 
                              This method should not be used from Client, but only for use from API.    
CreatedBy : Shyam
...
GO
IF  EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ws_spGetAllEventsForUserInternal]') 
    AND type in (N'P', N'PC'))
BEGIN
   PRINT 'DROPPING PROCEDURE ws_spGetAllEventsForUserInternal'
   DROP PROCEDURE [dbo].ws_spGetAllEventsForUserInternal
END
GO

PRINT 'Creating PROCEDURE ws_spGetAllEventsForUserInternal'
GO

CREATE Procedure [dbo].ws_spGetAllEventsForUserInternal
(
	@eventRequest XML
)
AS
BEGIN 
	...
	BEGIN 
	  SELECT @xmlc = (  		
						...
						upr.GCMClientId as InitiatorGCMClientId,ev.EventStateId,ev.TrackingStateId, ev.DestinationName, 
						...
						ev.IsRecurring, ev.RecurrenceFrequencyTypeId, ev.RecurrenceCount, ev.RecurrenceFrequency, ev.RecurrenceDaysOfWeek,							
						 ( SELECT ue.UserId , up.ProfileName, up.CountryCode, up.MobileNumber , up.GCMClientId, ue.EventAcceptanceStateId,
						  ue.IsTrackingAccepted, ue.UserEventStateId, ue.IsUserLocationShared
...
GO
PRINT 'PROCEDURE ws_spGetAllEventsForUserInternal created.'
GO
